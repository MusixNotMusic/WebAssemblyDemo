...
int MC33_faceTest1(int face, const float *v) {
	const int i[] = {0, 5, 1, 4, 1, 6, 2, 5, 3, 6, 2, 7, 0, 7, 3, 4, 0, 2, 1, 3, 4, 6, 5, 7};
	const int f[] = {0x48, 0x84, 0x24, 0x42, 0x21, 0x12, 0x18, 0x81, 0x50, 0xA0, 0x05, 0x0A};
	const int *j = i + (face << 2);
...
int MC33_faceTest1(int face, const float *v) {
	...
	switch (face) {
	case 0:
		i = (v[0]*v[5] < v[1]*v[4]? 0x48: 0x84);
		break;
	case 1:
		i = (v[1]*v[6] < v[2]*v[5]? 0x24: 0x42);
		break;
	case 2:
		i = (v[3]*v[6] < v[2]*v[7]? 0x21: 0x12);
		break;
	case 3:
		i = (v[0]*v[7] < v[3]*v[4]? 0x18: 0x81);
		break;
	case 4:
		i = (v[0]*v[2] < v[1]*v[3]? 0x50: 0xA0);
		break;
	default:
		i = (v[4]*v[6] < v[5]*v[7]? 0x05: 0x0A);
	}
	return i;
}
//*/
...
vertex label.
*/
unsigned int MC33_storePointNormal(MC33 *M, float *r) {
	unsigned int nv = M->nV++;
	float t, *p;
	if (nv == M->capv) {
		void *pt;
		M->capv <<= 1;
		pt = realloc(M->N,M->capv*3*sizeof(float));
		if (pt) {
			M->N = (float(*)[3])pt;
			pt = realloc(M->V,M->capv*3*sizeof(float));
			if (pt)
				M->V = (float(*)[3])pt;
			else
...
unsigned int MC33_surfint(MC33 *M, unsigned int x, unsigned int y, unsigned int z, float *r) {
	...
	return MC33_storePointNormal(M, r);
}
			...
			if (p[c] == FF) {
				...
				case 0:
					if (z || x)
						...
						if (v[0] == 0) {
							...
							p[0] = MC33_storePointNormal(M, u.r);
						}
						M->Dy[y][0] = p[0];
					}
					break;
				case 1:
					if (x)
						...
						if (v[1] == 0) {
							...
							u.r[5] = v[2] - v[1];
							p[1] = MC33_storePointNormal(M, u.r);
						}
						M->Lz[y + 1][0] = p[1];
					}
					break;
				case 2:
					if (x)
						...
						if (v[3] == 0) {
							...
							p[2] = MC33_storePointNormal(M, u.r);
						}
						M->Uy[y][0] = p[2];
					}
					break;
				case 3:
					if (y || x)
						...
						if (v[0] == 0) {
							...
							u.r[5] = v[3] - v[0];
							p[3] = MC33_storePointNormal(M,u.r);
						}
						M->Lz[0][0] = p[3];
					}
					break;
				case 4:
					if (z)
						...
						if (v[4] == 0) {
							...
							p[4] = MC33_storePointNormal(M,u.r);
						}
						M->Dy[y][x + 1] = p[4];
					}
					break;
				case 5:
					if (v[5] == 0) {
						...
						u.r[5] = v[6] - v[5];
						p[5] = MC33_storePointNormal(M,u.r);
					}
					M->Lz[y + 1][x + 1] = p[5];
					break;
				case 6:
					if (v[7] == 0) {
						...
						p[6] = MC33_storePointNormal(M,u.r);
					}
					M->Uy[y][x + 1] = p[6];
					break;
				case 7:
					if (y)
						...
						if (v[4] == 0) {
							...
							u.r[5] = v[7] - v[4];
							p[7] = MC33_storePointNormal(M,u.r);
						}
						M->Lz[0][x + 1] = p[7];
					}
					break;
				case 8:
					if (z || y)
						...
						if (v[0] == 0) {
							...
							p[8] = MC33_storePointNormal(M,u.r);
						}
						M->Dx[0][x] = p[8];
					}
					break;
				case 9:
					if (z)
						...
						if (v[1] == 0) {
							...
							p[9] = MC33_storePointNormal(M,u.r);
						}
						M->Dx[y + 1][x] = p[9];
					}
					break;
				case 10:
					if (v[2] == 0) {
						...
						p[10] = MC33_storePointNormal(M,u.r);
					}
					M->Ux[y + 1][x] = p[10];
					break;
				case 11:
					if (y)
						...
						if (v[3] == 0) {
							...
							p[11] = MC33_storePointNormal(M,u.r);
						}
						M->Ux[0][x] = p[11];
					}
				break;
				default:
					...
					p[12] = MC33_storePointNormal(M,u.r);
				}
			}
